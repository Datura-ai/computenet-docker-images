...
| `DOLPHIN_MAX_UNSERVED_SPAWNS` | no | `0` (off)                    | When every worker has failed this many spawns in a row without an engine answering `/health`, and none is serving, the container stops its workers and exits 0 (DAH-3475). Exit 0 is the contract with the validator's `restart: on-failure` policy for fillers (lium-io#1370): the container stays `exited` and the backend closes the run instead of dockerd restarting it. Under today's `unless-stopped` a zero exit is restarted too, so the cap ships off and the default becomes `8` once lium-io#1370 is deployed. One capped worker among serving siblings keeps respawning on its own. |
