...
`scripts/build-smoke.sh [template …]` resolves every template's bake file (`docker buildx bake --print`, and each default target's Dockerfile must exist) and builds and boots the templates a change touches; the CI job `build-smoke` runs it on every PR. Before a template is built its base images are pulled on their own (`--bases <template>` prints the list; each try under `T_PULL`, default 8m, three tries per image, all pulls of the template within `T_PULL_TOTAL`, default 25m), so a stalled registry ends with a `did not pull` / `pull budget spent` line instead of the 40-minute build timeout; the full build log lands in `artifacts/build-<template>.log`, which the CI job uploads even when the build times out. `E2E_GPU=1 scripts/build-smoke.sh pytorch` on a GPU host also requires `nvidia-smi` and `torch.cuda` inside the container. A template whose entrypoint exits without a GPU (`pearl-miner`) carries a `templates/<name>/smoke.gpu` marker: it is built on every host but booted only under `E2E_GPU=1`; on the CPU runner its boot row reads `SKIP`. Linux only: the script needs GNU `timeout` and `python3`.
